...
describe("prezzo inserito a mano", () => {
  ...
  it("Crea preventivo opens a PREVENTIVO PRONTO round at the typed price; nothing reaches the customer", async () => {
    ...
    const result = await enterManualPrice("tenant-1", manualRow.id, 28000, null, STAFF);
    ...
    expect(view?.view.customerPreview).toContain("Prezzo totale: 280,00 € per l'intero veicolo");
    expect(view?.view.customerPreview).toContain("Acconto per confermare la prenotazione: 140,00 €");
  });

  it("the new round is approved like any other, at the typed price, with the manual reason", async () => {
    const { tr, manualRow } = await notifyManualPrice();
    const created = await enterManualPrice("tenant-1", manualRow.id, 28000, null, STAFF);
    ...
    expect(modifyPriceForTransferRequest).toHaveBeenCalledWith(
      ...
      14000, // default deposit: 50% of 280, nearest 10 EUR
    );
    expect(state.customerSends).toBe(1);
  });
  ...
  it("Modifica and Rifiuta work on the new round too", async () => {
    ...
    const revised = await reviseQuoteRound("tenant-1", created.newRoundId!, 30000, null, { notify: false });
    expect(revised.outcome).toBe("done");
  ...
  it("a double click creates one round only and takes the second click to it", async () => {
    ...
    const first = await enterManualPrice("tenant-1", manualRow.id, 28000, null, STAFF);
    const second = await enterManualPrice("tenant-1", manualRow.id, 30000, null, STAFF);
  ...
  it("refuses an invalid price and changes nothing", async () => {
    const { tr, manualRow } = await notifyManualPrice();
    const result = await enterManualPrice("tenant-1", manualRow.id, 0, null, STAFF);
    expect(result.outcome).toBe("refused");
  ...
  it("respects the switches: off -> refused; customer outside the test list -> refused", async () => {
    ...
    process.env.QUOTE_APPROVAL_ENABLED = "false";
    expect((await enterManualPrice("tenant-1", manualRow.id, 28000, null, STAFF)).message).toContain("disattivato");

    process.env.QUOTE_APPROVAL_ENABLED = "true";
    process.env.QUOTE_APPROVAL_TEST_PHONES = "+393330000000";
    expect((await enterManualPrice("tenant-1", manualRow.id, 28000, null, STAFF)).message).toContain("numeri di prova");
  ...
  it("a deposit typed with the price is used for the round and the approval", async () => {
    const { manualRow } = await notifyManualPrice();
    const created = await enterManualPrice("tenant-1", manualRow.id, 28000, 10000, STAFF);

    const view = await getRoundForPanel("tenant-1", created.newRoundId!);
    expect(view?.view.depositLabel).toBe("100,00 €");
    await approveQuoteRound("tenant-1", created.newRoundId!, STAFF);
  ...
  it("refuses a deposit above the typed price", async () => {
    const { manualRow } = await notifyManualPrice();
