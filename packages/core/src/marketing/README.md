...
## Status: MIE-2 — real detection for GA4/GTM/Search Console/website/Google Ads. `@bos/ai` is the strategic-synthesis layer.
...
- **Check modules** (`checks/`), one real implementation each:
  ...
  - `google-ads-checks.ts` — Google Ads API v25, via `fetchGoogleAdsWeeklyPerformance` (`google-clients.ts`): zero conversions despite spend, high cost per conversion, low CTR. Requires `GOOGLE_ADS_DEVELOPER_TOKEN` (Basic Access approval from Google is a separate, external prerequisite for querying real, non-test accounts).
- **`run-check.ts`**: the orchestrator both the scheduled jobs and the on-demand button call. Runs every check against every linked resource (now including `google_ads_account`), **deduplicates** against already-open findings (via `evidence.dedupeKey`, updating `last_seen_at` instead of creating a duplicate every 4 hours), recomputes the Health Score snapshot, and sends a critical alert email if anything critical was found.
- **`strategist.ts`**: Claude-powered synthesis (structured tool-use output matching the findings schema) — runs on `daily_audit`/`on_demand` only, not `quick_check` (cost/latency discipline, per the original cadence design). System prompt explicitly forbids recommending automatic budget/campaign/bid/keyword execution. Invoked through `ai-analyst.ts` (below), not called directly by `run-check.ts` anymore.
- **`ai-analyst.ts`**: thin adapter making `@bos/ai`'s `google_marketing_analyst.agent` the single strategic-synthesis layer sitting above every deterministic check. Registers the agent once per process (`AgentOrchestrator`, module-level singleton) with `strategist.ts`'s `runStrategistSynthesis` injected as its implementation — `@bos/ai` itself has no dependency on this package's domain types, only on a loosely-typed `Record<string, unknown>[]` contract, to keep the package dependency graph one-directional (`@bos/core → @bos/ai`, never the reverse). Fails soft: any agent/orchestrator error, or a malformed agent output, is swallowed and logged rather than aborting the check run.
...
- **Google Ads campaign status, policy issues, disapproved ads, enhanced conversions** — `google-ads-checks.ts` covers zero-conversion spend, cost-per-conversion, and CTR only, using the same weekly aggregate query as everywhere else. Campaign-level/policy-level GAQL queries are a separate, larger slice, sequenced for later.
