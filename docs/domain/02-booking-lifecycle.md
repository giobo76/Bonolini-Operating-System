...
> - On `confirmed` (after the deposit) BOS creates the booking's event in the founder's Google Calendar; cancelling the booking in BOS marks that event "ANNULLATO" (grey), never deletes it. Details: `packages/core/src/calendar/README.md`.
